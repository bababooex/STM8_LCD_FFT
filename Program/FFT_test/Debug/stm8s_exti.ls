 ...
 449  0027 84            	pop	a
 450  0028 81            	ret
 495                     ; 126 EXTI_Sensitivity_TypeDef EXTI_GetExtIntSensitivity(EXTI_Port_TypeDef Port)
 495                     ; 127 {
 496                     .text:	section	.text,new
 497  0000               _EXTI_GetExtIntSensitivity:
 499  0000 88            	push	a
 500  0001 88            	push	a
 501       00000001      OFST:	set	1
 504                     ; 128   uint8_t value = 0;
 506  0002 0f01          	clr	(OFST+0,sp)
 508                     ; 131   assert_param(IS_EXTI_PORT_OK(Port));
 510  0004 4d            	tnz	a
 511  0005 2710          	jreq	L04
 512  0007 a101          	cp	a,#1
 513  0009 270c          	jreq	L04
 514  000b a102          	cp	a,#2
 515  000d 2708          	jreq	L04
 516  000f a103          	cp	a,#3
 517  0011 2704          	jreq	L04
 518  0013 a104          	cp	a,#4
 519  0015 2603          	jrne	L63
 520  0017               L04:
 521  0017 4f            	clr	a
 522  0018 2010          	jra	L24
 523  001a               L63:
 524  001a ae0083        	ldw	x,#131
 525  001d 89            	pushw	x
 526  001e ae0000        	ldw	x,#0
 527  0021 89            	pushw	x
 528  0022 ae0000        	ldw	x,#L111
 529  0025 cd0000        	call	_assert_failed
 531  0028 5b04          	addw	sp,#4
 532  002a               L24:
 533                     ; 133   switch (Port)
 535  002a 7b02          	ld	a,(OFST+1,sp)
 537                     ; 150   default:
 537                     ; 151     break;
 538  002c 4d            	tnz	a
 539  002d 270e          	jreq	L541
 540  002f 4a            	dec	a
 541  0030 2714          	jreq	L741
 542  0032 4a            	dec	a
 543  0033 271c          	jreq	L151
 544  0035 4a            	dec	a
 545  0036 2725          	jreq	L351
 546  0038 4a            	dec	a
 547  0039 2730          	jreq	L551
 548  003b 2035          	jra	L302
 549  003d               L541:
 550                     ; 135   case EXTI_PORT_GPIOA:
 550                     ; 136     value = (uint8_t)(EXTI->CR1 & EXTI_CR1_PAIS);
 552  003d c650a0        	ld	a,20640
 553  0040 a403          	and	a,#3
 554  0042 6b01          	ld	(OFST+0,sp),a
 556                     ; 137     break;
 ...
 580                     ; 143     break;
 582  005b 2015          	jra	L302
 583  005d               L351:
 584                     ; 144   case EXTI_PORT_GPIOD:
 584                     ; 145     value = (uint8_t)((uint8_t)(EXTI->CR1 & EXTI_CR1_PDIS) >> 6);
 586  005d c650a0        	ld	a,20640
 587  0060 a4c0          	and	a,#192
 588  0062 4e            	swap	a
 589  0063 44            	srl	a
 590  0064 44            	srl	a
 591  0065 a403          	and	a,#3
 592  0067 6b01          	ld	(OFST+0,sp),a
 594                     ; 146     break;
 596  0069 2007          	jra	L302
 597  006b               L551:
 598                     ; 147   case EXTI_PORT_GPIOE:
 598                     ; 148     value = (uint8_t)(EXTI->CR2 & EXTI_CR2_PEIS);
 600  006b c650a1        	ld	a,20641
 601  006e a403          	and	a,#3
 602  0070 6b01          	ld	(OFST+0,sp),a
 604                     ; 149     break;
 606  0072               L751:
 607                     ; 150   default:
 607                     ; 151     break;
 609  0072               L302:
 610                     ; 154   return((EXTI_Sensitivity_TypeDef)value);
 612  0072 7b01          	ld	a,(OFST+0,sp)
 615  0074 85            	popw	x
 616  0075 81            	ret
 650                     ; 162 EXTI_TLISensitivity_TypeDef EXTI_GetTLISensitivity(void)
 650                     ; 163 {
 651                     .text:	section	.text,new
 652  0000               _EXTI_GetTLISensitivity:
 654  0000 88            	push	a
 655       00000001      OFST:	set	1
 658                     ; 164   uint8_t value = 0;
 660                     ; 167   value = (uint8_t)(EXTI->CR2 & EXTI_CR2_TLIS);
 662  0001 c650a1        	ld	a,20641
 663  0004 a404          	and	a,#4
 664  0006 6b01          	ld	(OFST+0,sp),a
 666                     ; 169   return((EXTI_TLISensitivity_TypeDef)value);
 668  0008 7b01          	ld	a,(OFST+0,sp)
 671  000a 5b01          	addw	sp,#1
 672  000c 81            	ret
 685                     	xdef	_EXTI_GetTLISensitivity
 686                     	xdef	_EXTI_GetExtIntSensitivity
 687                     	xdef	_EXTI_SetTLISensitivity
 688                     	xdef	_EXTI_SetExtIntSensitivity
 689                     	xdef	_EXTI_DeInit
 690                     	xref	_assert_failed
 691                     .const:	section	.text
 692  0000               L111:
 693  0000 7372635c7374  	dc.b	"src\stm8s_exti.c",0
 713                     	end
